...
module.exports = {  
  ...
  corner1st: async (req, res) => {
    const table = req.params['leagueID'].replace('-', '_').toLowerCase()    
    const result = await pg.pool.query({        
      text: `WITH corners AS (
        SELECT game_date, team_home AS "team", corners_home_ht AS "FOR", corners_away_ht AS "AGAINST"          
        FROM "${table}"
        UNION ALL
        SELECT game_date, team_away, corners_away_ht, corners_home_ht
        FROM "${table}"
        )
        SELECT "team",
          COUNT("team") AS "g",    
  ...
  perDomain: async (req, res) => {
    ...
    const op = {
      "home": ['team_home', 'corners_home_ft', 'corners_away_ft'],
      "away": ['team_away','corners_away_ft', 'corners_home_ft']
    }
    const result = await pg.pool.query({        
  ...
  perDomain1st: async (req, res) => {
    const table = req.params['leagueID'].replace('-', '_').toLowerCase()
    const domain = req.params['domain'].toLowerCase()
    const op = {
      "home": ['team_home', 'corners_home_ht', 'corners_away_ht'],
      "away": ['team_away', 'corners_away_ht', 'corners_home_ht']
    }
    const result = await pg.pool.query({        
      text: `SELECT ${op[domain][0]} AS "team", 
              COUNT(${op[domain][0]}) AS "g", 
